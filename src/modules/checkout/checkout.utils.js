import { dateFormat, ignoreLogger, ProductCode, VnpLocale } from "vnpay";
...
export const generateVnpayLink = async (orderId, amount, userId) => {
  ...
  const exprireDate = new Date(createDate.getTime() + 5 * 60 * 1000);
  const vnp_ExpireDate = dateFormat(exprireDate);
